...
/*
| raw | content | meaning |
| - | tag: announcements.trafficEvent | type
| d.TycodeIt, "_", d.SubTycodeIt), " | ", joinStr(d.TycodeDe, "_", d.SubTycodeDe | shortname | display name |
| ?? | severity | severity of the event | <- new
| map from tycodeValue | tag: trafficEvent.[type] | type of the traffic event |
| map from subTycodeValue | tag: trafficEvent.[subtype] | subtype of the traffic event |
| d.TycodeIt+d.SubTycodeIt / d.TycodeDe + d.SubTycodeDe | Details.Title | what happened |
| placeDe / placeIt | Details.BaseText | description of the event |
...

{
	...
	tag: [
		{
			announcements.traffic-event,
			trafficEvent.incident
		}
	]
}
...
func MapTrafficEventToAnnouncement(tags Tags, raw dto.TrafficEvent, id string) (odhContentModel.Announcement, error) {
	...
	// GPS Info (Position)
	// Default
	announcement.WKTGeometry4326 = PROV_BZ_WKT
	...
	if raw.X != nil && raw.Y != nil {
		...
		if xIsFloat && yIsFloat {
			...
			if xFloat != 0 && yFloat != 0 {
				announcement.WKTGeometry4326 = fmt.Sprintf("POINT (%f %f)", xFloat, yFloat)
			}
		}
	}
