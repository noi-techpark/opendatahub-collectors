...
public class ChargePusher extends NonBlockingJSONPusher {
	...
	public <T> DataMapDto<RecordDtoImpl> mapData(Date timestamp, T rawData) {
		if (rawData == null)
			return null;
		...
		for (ChargerDtoV2 dto : data) {
			...
			SimpleRecordDto rec = new SimpleRecordDto(timestamp.getTime(), availableStations.doubleValue(), period);
			records.add(rec);
	...
	public DataMapDto<RecordDtoImpl> mapPlugData2Bdp(Date timestamp, List<ChargerDtoV2> data) {
		if (data == null)
			return null;
		Integer period = env.getProperty("app_period", Integer.class);
		...
		for (ChargerDtoV2 dto : data) {
			...
			for (ChargingPointsDtoV2 point : dto.getChargingPoints()) {
				if (point.getState() != null) {
					...
					rec.setTimestamp(timestamp.getTime());
