...
const GEOMETRY_FORMAT = "esrijson"
...
func pagedURL(base *url.URL, offset int) string {
	urlCopy := *base
	q := urlCopy.Query()
	q.Set("geometryFormat", GEOMETRY_FORMAT)
...
func main() {
	...
	c.AddFunc(env.CRON, func() {
		...
		for offset := 0; ; offset += pageSize() {
			...
			req, err := http.NewRequest(env.HTTP_METHOD, pageURL, http.NoBody)
			...
			if resp.StatusCode != http.StatusOK {
				slog.Error("non-OK HTTP status", "statusCode", resp.StatusCode)
				return
			}
